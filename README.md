...
<style>
  ...
  button.main-btn:hover { background: #3a3a3a; transform: translateY(-2px); }
  ...
  .type-chillsear { color: #fff; text-shadow: 0 0 15px var(--chill-blue); }
...
<div id="modal">
  ...
  <button onclick="closeModal()" style="background:#444; border:none; color:white; padding:5px 15px; border-radius:5px;">닫기</button>
</div>
...
<div id="game-container">
  ...
  <div id="result">
    ...
    <button onclick="nextPlayer()" style="background: #111; color: #fff; width: 100%; padding: 15px; border-radius: 10px; border: none;">확인 완료</button>
  </div>

  <div id="final-screen">
    <div style="color: #444; margin-bottom: 20px;">ROUND ENDED</div>
    <div id="finalType" class="final-murder-type"></div>
    <div id="finalDetail" style="font-size: 22px; color: #555; font-weight: bold;"></div>
    <br><br>
    <button onclick="location.reload()" style="background: none; border: 1px solid #222; color: #222; font-size: 12px;">RESTART</button>
  </div>
</div>
...
const murderTypes = [
  { name: "Standard", weight: 60, desc: "추가 능력이 포함된 기본 라운드였습니다.", class: "type-standard" },
  { name: "Chillsear", weight: 30, desc: "의사가 치료할 수 없는 무자비한 라운드였습니다.", class: "type-chillsear" },
  { name: "Zombie", weight: 10, desc: "죽은 자들이 좀비가 되는 감염 라운드였습니다.", class: "type-zombie" }
];
...
function startGame() {
  ...
  for (const t of murderTypes) {
    cum += t.weight;
    if (rand < cum) { roundMurderType = t; break; }
  }
